...
def create_job_file(property, input_file, output_file, epoch, guidance=0.25):
    ...
    exp_name = f"eval_qual_prop_{property}_w_{guidance}_ckpt_{epoch}"
...
properties = ['alpha', 'mu', 'Cv', 'gap', 'homo', 'lumo']
guidance_weights = [2]
...
for property in properties:
    ...
    for epoch in epochs:
        for guidance in guidance_weights:
            output_file = f'job_drafts_qual/evals_{property}_w_{guidance}_qual_prop.sh'
            input_file = 'experiments/template_evals_ckpt_qual.sh'
            if args.resume:
                output_file = f'job_drafts_qual/evals_{property}_w_{guidance}_{epoch}_qual_prop_resume.sh'
                input_file = 'experiments/template_evals_resume_ckpt_qual.sh'
