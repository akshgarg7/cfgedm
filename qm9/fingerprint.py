...
def compute_fingerprint(poss,numberss): #num_atomss
    return compute_fingerprint_bits(poss,numberss)[1]

def compute_fingerprint_bits(poss,numberss): ##num_atomss
    #assert(len(poss) == len(numberss) == len(num_atomss))
    fingerprint_1024 = []
    fingerprint_bits = []
    #ids = len(num_atomss)
    for i in range(poss.size(0)):
        pos = poss[i, :]
        numbers = numberss[i, :].squeeze()
        num_atoms = poss.size(1) #num_atomss[i]
